document.addEventListener("DOMContentLoaded", () => {
  ...
  const config = {
    ...
    maxStoredMessages: 50, // Maximum number of messages to store
  };
  ...
  function loadMessages() {
    const savedMessages = localStorage.getItem("jarvisChatMessages");
    if (savedMessages) {
      ...
      messages.forEach((msg) => {
        ...
        // Recreate the message in the chat interface
        addMessage(msg.sender, formattedText, false, true);
      });
  ...
  function saveMessages(sender, text) {
    let messages = JSON.parse(
      localStorage.getItem("jarvisChatMessages") || "[]"
    );

    // Strip any existing HTML tags before saving
    ...
    if (messages.length > config.maxStoredMessages) {
      messages = messages.slice(messages.length - config.maxStoredMessages);
    }

    localStorage.setItem("jarvisChatMessages", JSON.stringify(messages));
  }
  ...
  // Load settings and messages when page loads
  loadSettings();
  loadMessages();
